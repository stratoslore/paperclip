...
상태: 첫 릴리스(V1)를 위한 구현 계약
Date: 2026-02-17
대상: 제품, 엔지니어링, 에이전트 통합 작성자
Source inputs: `GOAL.md`, `PRODUCT.md`, `SPEC.md`, `DATABASE.md`, current monorepo code

## 1. Document Role

`SPEC.md`는 장기 제품 스펙으로 유지됩니다.
이 문서는 구체적이고 빌드 준비가 된 V1 계약입니다.
충돌이 있는 경우, `SPEC-implementation.md`가 V1 동작을 제어합니다.

## 2. V1 Outcomes

Paperclip V1은 자율 에이전트를 위한 전체 컨트롤 플레인 루프를 제공해야 합니다:

1. 인간 보드가 회사를 생성하고 목표를 정의합니다.
2. 보드가 조직 트리에서 에이전트를 생성하고 관리합니다.
3. 에이전트가 하트비트 호출을 통해 작업을 수신하고 실행합니다.
4. 모든 작업은 감사 가시성이 있는 작업/댓글을 통해 추적됩니다.
5. 토큰/비용 사용량이 보고되고 예산 제한이 작업을 중단할 수 있습니다.
6. 보드는 어디서든 개입할 수 있습니다 (에이전트/작업 일시정지, 결정 오버라이드).

성공은 한 운영자가 명확한 가시성과 제어로 소규모 AI 네이티브 회사를 엔드투엔드로 운영할 수 있음을 의미합니다.

## 3. Explicit V1 Product Decisions

이 결정들은 V1을 위해 `SPEC.md`의 미해결 질문을 종결합니다.

| Topic | V1 Decision |
|---|---|
| Tenancy | Single-tenant deployment, multi-company data model |
| 회사 model | 회사 is first-order; all business entities are company-scoped |
...
V1 구현은 이 기준선을 회사 중심, 거버넌스 인식 컨트롤 플레인으로 확장합니다.

## 5. V1 범위

## 5.1 In 범위

- 회사 lifecycle (create/list/get/update/archive)
...
- 플러그인 framework and third-party extension SDK
...
별도 언급이 없으면 모든 코어 테이블은 `id`, `created_at`, `updated_at`를 포함합니다.
...
불변성: 모든 비즈니스 레코드는 정확히 하나의 회사에 속합니다.
...
## 10.8 Activity and 대시보드

- `GET /companies/:companyId/activity`
- `GET /companies/:companyId/dashboard`

대시보드 payload must include:
...
4. 에이전트 can update tasks/comments and report costs with API keys only.
...
7. 대시보드 shows accurate counts/spend from live DB data.
...
## 21. 회사 Portability Package (V1 Addendum)
