...
- 플러그인 UI bundles currently run as same-origin JavaScript inside the main Paperclip app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
...
2. 플러그인 installation is global to the instance.
...
5. 프로젝트 already have a real workspace model via `project_workspaces`, and local/runtime plugins should build on that instead of inventing a separate workspace abstraction.
...
플러그인s resolve workspace paths from this model to locate local directories for file, terminal, git, and process operations.
...
플러그인s may only call host APIs that are covered by granted capabilities.
...
플러그인s that need local tooling (file browsing, git, terminals, process tracking) can resolve workspace paths through the project workspace APIs and then operate on the filesystem, spawn processes, and run git commands directly. The host does not wrap these operations — plugins own their own implementations.

## 8. Installation Model

플러그인 installation is global and operator-driven.
...
플러그인s live under the Paperclip instance directory.
...
플러그인s may contribute tools that Paperclip agents can use during runs.

### 11.1 도구 선언

플러그인s declare tools in their manifest:
...
플러그인 tools appear in the agent's tool list alongside core tools but are visually distinguished in the UI as plugin-contributed.

### 11.4 Constraints

- 플러그인 tools must not override or shadow core tools by name.
- 플러그인 tools must be idempotent where possible.
...
플러그인s do not talk to the DB directly.
플러그인s do not read raw secret material from persisted config.
...
`ctx.data` and `ctx.actions` register handlers that the plugin's own UI calls through the host bridge. `ctx.data.register(key, handler)` backs `use플러그인Data(key)` on the frontend. `ctx.actions.register(key, handler)` backs `use플러그인Action(key)`.

플러그인s that need filesystem, git, terminal, or process operations handle those directly using standard Node APIs or libraries. The host provides project workspace metadata through `ctx.projects` so plugins can resolve workspace paths, but the host does not proxy low-level OS operations.
...
플러그인s may provide an optional filter when subscribing to events. The filter is evaluated by the host before dispatching to the worker, so filtered-out events never cross the process boundary.
...
### 16.2 플러그인-to-플러그인 Events

플러그인s may emit custom events using `ctx.events.emit(name, payload)`. 플러그인-emitted events use a namespaced event type: `plugin.<pluginId>.<eventName>`.
...
- 플러그인 events require the `events.emit` capability.
- 플러그인 events are not core domain events — they do not appear in the core activity log unless the emitting plugin explicitly logs them.
- 플러그인 events follow the same at-least-once delivery semantics as core events.
- The host must not allow plugins to emit events in the core namespace (events without the `plugin.` prefix).

## 17. Scheduled Jobs

플러그인s may declare scheduled jobs in their manifest.
...
플러그인s may declare webhook endpoints in their manifest.
...
플러그인s ship their own frontend UI as a bundled React module. The host loads plugin UI into designated extension slots and provides a bridge for the plugin frontend to communicate with its own worker backend and with host APIs.

### How 플러그인 UI Publishing Works In Practice
...
1. User opens the dashboard. The host sees that the Linear plugin registered a `대시보드Widget` export.
2. The host mounts the plugin's `대시보드Widget` component into the dashboard widget slot, passing `context` (current company, user, etc.) and the bridge.
3. `use플러그인Data("sync-health", ...)` calls through the bridge → host → plugin worker's `getData` RPC → returns JSON → the plugin component renders it however it wants.
4. When the user clicks "Resync Now", `use플러그인Action("resync")` calls through the bridge → host → plugin worker's `performAction` RPC.
...
### 19.0.1 플러그인 UI SDK (`@paperclipai/plugin-sdk/ui`)

The SDK includes a `ui` subpath export that plugin frontends import. This subpath provides:

- **Bridge hooks**: `use플러그인Data(key, params)`, `use플러그인Action(key)`, `useHostContext()`
...
- **Type definitions**: `플러그인PageProps`, `플러그인WidgetProps`, `플러그인DetailTabProps`

플러그인s are encouraged but not required to use the shared components. A plugin may render entirely custom UI as long as it communicates through the bridge.

### 19.0.2 Bundle Isolation

플러그인 UI bundles are loaded as standard ES modules, not iframed. This gives plugins full rendering performance and access to the host's design tokens.

Isolation rules:

- 플러그인 bundles must not import from host internals. They may only import from `@paperclipai/plugin-sdk/ui` and their own dependencies.
- 플러그인 bundles must not access `window.fetch` or `XMLHttpRequest` directly for host API calls. All host communication goes through the bridge.
- The host may enforce Content Security Policy rules that restrict plugin network access to the bridge endpoint only.
- 플러그인 bundles must be statically analyzable — no dynamic `import()` of URLs outside the plugin's own bundle.
...
플러그인 UI bundles must be pre-built ESM. The host does not compile or transform plugin UI code at runtime.
...
플러그인s may add cards or sections to the dashboard.

## 19.5 사이드바 항목

플러그인s may add sidebar links to:
...
| `ActionBar` | Row of buttons wired to `use플러그인Action` | Resync, create branch, restart process |
...
플러그인s may also use entirely custom components. The shared components exist to reduce boilerplate and keep visual consistency, not to limit what plugins can render.
...
`use플러그인Action` returns an async function that either resolves with the result or throws a `플러그인BridgeError`.

`플러그인BridgeError` shape:
...
플러그인s that need filesystem, git, terminal, or process operations implement those directly. The host does not wrap or proxy these operations.

The host provides workspace metadata through `ctx.projects` (list workspaces, get primary workspace, resolve workspace from issue or agent/run). 플러그인s use this metadata to resolve local paths and then operate on the filesystem, spawn processes, shell out to `git`, or open PTY sessions using standard Node APIs or any libraries they choose.

This keeps the host lean — it does not need to maintain a parallel API surface for every OS-level operation a plugin might need. 플러그인s own their own logic for file browsing, git workflows, terminal sessions, and process management.
...
3. 플러그인 data should scope to existing Paperclip objects before new tables are introduced.
...
플러그인-originated mutations should write:

- `actor_type = plugin`
- `actor_id = <plugin-id>`

## 21.5 플러그인 Migrations
...
플러그인 config must never persist raw secret values.

Rules:

1. 플러그인 config stores secret refs only.
2. Secret refs resolve through the existing Paperclip secret provider system.
3. 플러그인 workers receive resolved secrets only at execution time.
...
3. 플러그인-owned data (`plugin_state`, `plugin_entities`, `plugin_jobs`, `plugin_job_runs`, `plugin_webhook_deliveries`, `plugin_config`) is retained for a configurable grace period (default: 30 days).
...
플러그인 upgrades do not automatically migrate plugin state. If a plugin's `value_json` shape changes between versions:
...
- 플러그인s should version their state keys or use a schema version field inside `value_json` to detect and handle format changes.
...
### 25.4 Hot 플러그인 Lifecycle

플러그인 install, uninstall, upgrade, and config changes **must** take effect without restarting the Paperclip server. This is a normative requirement, not optional.
...
## 26. 플러그인 Observability

### 26.1 로깅

플러그인 workers use `ctx.logger` to emit structured logs. The host captures these logs and stores them in a queryable format.

Log storage rules:

- 플러그인 logs are stored in a `plugin_logs` table or appended to a log file under the plugin's data directory.
...
## 27. 플러그인 Development And Testing
...
2. 플러그인 manifest declares exactly one `apiVersion`.
3. Host rejects unsupported versions at install time.
4. 플러그인 upgrades are explicit operator actions.
...
- `@paperclipai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `define플러그인`, `z`)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@paperclipai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. 플러그인s built with SDK 1.x continue to declare `apiVersion: 1`.
...
4. **Minimum SDK version in manifest**: 플러그인s declare `sdkVersion` in the manifest as a semver range (e.g. `">=1.4.0 <2.0.0"`). The host validates this at install time and warns if the plugin's declared range is outside the host's supported SDK versions.
...
### 29.4 플러그인 Author Workflow

When a new SDK version is released:

1. 플러그인 author updates `@paperclipai/plugin-sdk` dependency.
2. 플러그인 author follows the migration guide to update code.
3. 플러그인 author updates `apiVersion` and `sdkVersion` in the manifest.
4. 플러그인 author publishes a new plugin version.
...
- bridge error propagation (`플러그인BridgeError`)
