...
Paperclip의 UI는 전문가급 컨트롤 플레인입니다, not a toy dashboard. It should feel like the kind of tool you live in all day — fast, keyboard-driven, information-dense without being cluttered, dark-themed by default. Every pixel should earn its place.
...
**회사 switcher** is a dropdown button that occupies the full width of the sidebar header. It shows:
- 회사 icon (first letter avatar with company color, or uploaded icon)
- 회사 name (truncated with ellipsis if long)
- Chevron-down icon
...
- **받은 편지함** — items requiring the board operator's attention. Badge count on the right. Includes: pending approvals, budget alerts, failed heartbeats. The number is the total unread/unresolved count.
...
- **프로젝트** — project list. 프로젝트 group issues and link to goals.
- **목표** — company goal hierarchy.
...
- **대시보드** — company health overview: agent statuses, task velocity, cost burn, pending approvals count.
- **조직도** — interactive tree visualization of the agent reporting hierarchy.
- **에이전트** — flat list of all agents with status, role, last heartbeat, spend.
- **비용** — cost dashboard with breakdowns by agent, project, model, time.
- **Activity** — audit log of all system events.

Note: Approvals do not have a top-level sidebar entry. They are surfaced through the **받은 편지함** (primary interaction point), **대시보드** (pending count metric), and **inline on entity pages** (e.g., an agent detail page shows the approval that authorized its hire). The `/approvals` route still exists and is reachable via "See all approvals" links in 받은 편지함 and 대시보드, but it is not in the sidebar navigation.
...
목표 are displayed as a hierarchical tree, since goals have parent-child relationships.
...
Each row: expand chevron (if has children), target icon, title, level badge (회사/Team/Agent/Task), status badge.
...
The dashboard is the company health overview. Shown when clicking "대시보드" in the 회사 section.
...
1. **에이전트** — total, active, running, paused, error counts. Each with colored dots.
2. **Tasks** — open, in progress, blocked, done counts.
3. **비용** — month-to-date spend in dollars, budget utilization percentage with a mini progress bar.
4. **Approvals** — pending count (clickable to navigate to 받은 편지함, which is the primary approval interaction point).
...
**Breadcrumb tabs:** Overview | Heartbeats | Issues | 비용
...
**비용 tab:** cost breakdown for this agent — by model, by time period, with budget progress bar.
...
**1. 받은 편지함 (primary).** Pending approvals are the highest-priority inbox items. The board operator sees them front and center with inline approve/reject actions (see Section 14).

**2. 대시보드 metric card.** The "Pending Approvals" card shows the count and links to the full approvals list.
...
### 11.2 승인 목록 페이지 (`/approvals`)

This page still exists — it's the "See all" destination from 받은 편지함 and 대시보드. But it's not in the sidebar.
...
- 받은 편지함 is computed from live data (pending approvals query + alert conditions), not a separate notification table. This keeps it simple for V1.
...
7. 이슈 상세 view (three-pane with properties panel)
8. New issue modal
9. Issue comments
...
### Phase 4: 회사-Level Views
15. 받은 편지함 with inline approval actions (primary approval UX)
16. 대시보드 redesign with metric cards
17. Org chart interactive visualization
18. Cost dashboard
19. Activity log with filtering
20. Approvals list page (accessed via 받은 편지함 "See all", not sidebar)
