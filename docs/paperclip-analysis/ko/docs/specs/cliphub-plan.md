# ClipHub: Paperclip 팀 설정 마켓플레이스

> 대체 참고: 이 마켓플레이스 계획은 다음보다 이전입니다 the markdown-first company package direction. For the current package-format and import/export rollout plan, see `doc/plans/2026-03-13-company-import-export-v2.md` and `docs/companies/companies-spec.md`.

> 전체 회사 AI 팀을 위한 "앱 스토어" — pre-built Paperclip configurations, agent blueprints, skills, and governance templates that ship real work from day one.
...
- **Org chart**: 에이전트 with roles, titles, reporting chains, capabilities
...
### 2.2 에이전트 블루프린트 (팀 컨텍스트 내 개별 에이전트)
...
2. **조직도**: Interactive visualization of agent hierarchy
...
5. **Included 프로젝트**: Project templates with workspace configs
...
When a buyer clicks "Install to 회사":
