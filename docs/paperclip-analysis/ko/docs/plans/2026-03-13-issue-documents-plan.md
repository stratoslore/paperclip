...
Paperclip에 일급 **문서**를 추가합니다 as editable, revisioned, company-scoped text artifacts that can be linked to issues.

첫 번째 필수 규칙은 키가 `plan`인 문서입니다.

이것은 즉각적인 워크플로우 문제를 해결합니다 in `PAP-448`:

- 계획은 이슈 설명 안에서 더 이상 살지 않아야 합니다 as `<plan>` blocks
- 에이전트와 보드 사용자가 이슈 문서를 직접 생성/업데이트할 수 있어야 합니다
...
이것은 **텍스트 문서 조각**으로 구축되어야 합니다 of the broader artifact system, not as a replacement for attachments/assets.
