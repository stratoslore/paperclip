...
에이전트 회사 패키지는 파일시스템 및 GitHub 네이티브 형식입니다 for describing a company, team, agent, project, task, and associated skills using markdown files with YAML frontmatter.

이 사양은 에이전트 스킬 사양의 확장이지, 대체가 아닙니다.

It defines how company-, team-, and agent-level package structure composes around the existing `SKILL.md` model.

이 사양은 벤더 중립적입니다. It is intended to be usable by any agent-company runtime, not only Paperclip.
...
3. 레지스트리는 선택적 검색 레이어이며, 권한이 아닙니다.
