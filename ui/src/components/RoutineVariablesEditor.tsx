  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger className="flex w-full items-center justify-between rounded-lg border border-border/70 px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("variables.title", { defaultValue: "Variables" })}</p>
          <p className="text-xs text-muted-foreground">
            {t("variables.detected_hint", { defaultValue: "Detected from `{{name}}` placeholders in the routine title and instructions.", interpolation: { escapeValue: false } })}
          </p>
        </div>
          ...
          <div key={variable.name} className="rounded-lg border border-border/70 p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("variables.prompt_hint", { defaultValue: "Prompt the user for this value before each manual run." })}
              </span>
            </div>

            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("variables.label", { defaultValue: "Label" })}</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{t("variables.type", { defaultValue: "Type" })}</Label>
                <Select
                  value={variable.type}
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  <Label className="text-xs">{t("variables.default_value", { defaultValue: "Default value" })}</Label>
                  <label className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    {t("variables.required", { defaultValue: "Required" })}
                  </label>
                </div>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unset__">{t("variables.no_default", { defaultValue: "No default" })}</SelectItem>
                      <SelectItem value="true">{t("variables.true", { defaultValue: "True" })}</SelectItem>
                      <SelectItem value="false">{t("variables.false", { defaultValue: "False" })}</SelectItem>
                    </SelectContent>
                  </Select>
                ) : variable.type === "select" ? (
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("variables.options", { defaultValue: "Options" })}</Label>
                      <Input
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("variables.default_option", { defaultValue: "Default option" })}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t("variables.no_default", { defaultValue: "No default" })} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__unset__">{t("variables.no_default", { defaultValue: "No default" })}</SelectItem>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? "42" : t("variables.default_value", { defaultValue: "Default value" })}
                  />
                )}
              </div>
            </div>
...
export function RoutineVariablesHint() {
  ...
  return (
    <div className="rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
      {t("variables.placeholder_hint", { defaultValue: "Use `{{variable_name}}` placeholders in the instructions to prompt for inputs when the routine runs.", interpolation: { escapeValue: false } })}
    </div>
  );
}
