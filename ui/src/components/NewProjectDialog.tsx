      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>{t("newProject.heading", { defaultValue: "New project" })}</span>
          </div>
        ...
        <div className="px-4 pt-4 pb-2 shrink-0">
          <input
            ...
            placeholder={t("newProject.namePlaceholder", { defaultValue: "Project name" })}
            value={name}
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("newProject.repoUrl", { defaultValue: "Repo URL" })}</label>
              <span className="text-xs text-muted-foreground/50">{t("newProject.optional", { defaultValue: "optional" })}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("newProject.repoUrlTooltip", { defaultValue: "Link a GitHub repository so agents can clone, read, and push code for this project." })}
                </TooltipContent>
              </Tooltip>
            </div>
            <input
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("newProject.localFolder", { defaultValue: "Local folder" })}</label>
              <span className="text-xs text-muted-foreground/50">{t("newProject.optional", { defaultValue: "optional" })}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                ...
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("newProject.localFolderTooltip", { defaultValue: "Set an absolute path on this machine where local agents will read and write files for this project." })}
                </TooltipContent>
              </Tooltip>
            </div>
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t("newProject.plusGoal", { defaultValue: "+ Goal" }) : t("newProject.goal", { defaultValue: "Goal" })}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-56 p-1" align="start">
              {selectedGoals.length === 0 && (
                ...
                >
                  {t("newProject.noGoal", { defaultValue: "No goal" })}
                </button>
              )}
              ...
              {selectedGoals.length > 0 && availableGoals.length === 0 && (
                <div className="px-2 py-1.5 text-xs text-muted-foreground">
                  {t("newProject.allGoalsSelected", { defaultValue: "All goals already selected." })}
                </div>
              )}
            </PopoverContent>
          </Popover>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("newProject.createError", { defaultValue: "Failed to create project." })}</p>
          ) : (
            <span />
          )}
          <Button
          ...
          >
            {createProject.isPending ? t("newProject.creating", { defaultValue: "Creating…" }) : t("newProject.createProject", { defaultValue: "Create project" })}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
