...
function SkillList({ values }: { values: unknown }) {
  ...
  // useTranslation not needed here — no user-visible text to translate
  const items = values
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{i18n.t("approvals:payload.skills", { defaultValue: "Skills" })}</span>
...
export function HireAgentPayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      <div className="flex items-center gap-2">
        <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{t("payload.name", { defaultValue: "Name" })}</span>
        <span className="font-medium">{String(payload.name ?? "—")}</span>
      </div>
      <PayloadField label={t("payload.role", { defaultValue: "Role" })} value={payload.role} />
      <PayloadField label={t("payload.title", { defaultValue: "Title" })} value={payload.title} />
      <PayloadField label={t("payload.icon", { defaultValue: "Icon" })} value={payload.icon} />
      {!!payload.capabilities && (
        <div className="flex items-start gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{t("payload.capabilities", { defaultValue: "Capabilities" })}</span>
          <span className="text-muted-foreground">{String(payload.capabilities)}</span>
        </div>
      )}
      {!!payload.adapterType && (
        <div className="flex items-center gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{t("payload.adapter", { defaultValue: "Adapter" })}</span>
...
export function BudgetOverridePayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      <PayloadField label={t("payload.scope", { defaultValue: "Scope" })} value={payload.scopeName ?? payload.scopeType} />
      <PayloadField label={t("payload.window", { defaultValue: "Window" })} value={payload.windowKind} />
      <PayloadField label={t("payload.metric", { defaultValue: "Metric" })} value={payload.metric} />
