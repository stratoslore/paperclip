    ...
    <Card>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(row.inputTokens + row.cachedInputTokens)}</span> {t("billerSpend.in", { defaultValue: "in" })}
              {" · "}
              <span className="font-mono">{formatTokens(row.outputTokens)}</span> {t("billerSpend.out", { defaultValue: "out" })}
              {" · "}
              {t("billerSpend.providerCount", { defaultValue: "{{count}} provider", count: row.providerCount })}
              {" · "}
              {t("billerSpend.modelCount", { defaultValue: "{{count}} model", count: row.modelCount })}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {budgetMonthlyCents > 0 && (
          <QuotaBar
            label={t("billerSpend.periodSpend", { defaultValue: "Period spend" })}
            percentUsed={budgetPct}
            leftLabel={formatCents(row.costCents)}
            rightLabel={t("billerSpend.percentOfAllocation", { defaultValue: "{{percent}}% of allocation", percent: Math.round(budgetPct) })}
          />
        )}

        <div className="text-xs text-muted-foreground">
          {t("billerSpend.meteredRunCount", { defaultValue: "{{count}} metered run", count: row.apiRunCount })}
          {" · "}
          {t("billerSpend.subscriptionRunCount", { defaultValue: "{{count}} subscription run", count: row.subscriptionRunCount })}
          {" · "}
          {t("billerSpend.thisWeek", { defaultValue: "{{amount}} this week", amount: formatCents(weekSpendCents) })}
        </div>
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("billerSpend.billingTypes", { defaultValue: "Billing types" })}
              </p>
        ...
        {providerBreakdown.length > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("billerSpend.upstreamProviders", { defaultValue: "Upstream providers" })}
              </p>
              <div className="space-y-1.5">
                  ...
                  <div key={entry.provider} className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <div className="text-right tabular-nums">
                      ...
                      <div className="text-muted-foreground">
                        {formatTokens(entry.inputTokens + entry.outputTokens)} {t("billerSpend.tok", { defaultValue: "tok" })}
                      </div>
                    </div>
                  </div>
                ))}
