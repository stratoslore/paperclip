        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(totalInputTokens)}</span> {t("providerQuota.in", { defaultValue: "in" })}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span> {t("providerQuota.out", { defaultValue: "out" })}
              {(totalApiRuns > 0 || totalSubRuns > 0) && (
                <span className="ml-1.5">
                  ·{" "}
                  {totalApiRuns > 0 && t("providerQuota.apiRuns", { defaultValue: "~{{count}} api", count: totalApiRuns })}
                  {totalApiRuns > 0 && totalSubRuns > 0 && " / "}
                  {totalSubRuns > 0 && t("providerQuota.subRuns", { defaultValue: "~{{count}} sub", count: totalSubRuns })}
                  {" "}{t("providerQuota.runs", { defaultValue: "runs" })}
                </span>
              )}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        {hasBudget && (
          <div className="space-y-3">
            <QuotaBar
              label={t("providerQuota.periodSpend", { defaultValue: "Period spend" })}
              percentUsed={budgetPct}
              leftLabel={formatCents(totalCostCents)}
              rightLabel={t("providerQuota.percentOfAllocation", { defaultValue: "{{percent}}% of allocation", percent: Math.round(budgetPct) })}
              showDeficitNotch={showDeficitNotch}
            />
            <QuotaBar
              label={t("providerQuota.thisWeek", { defaultValue: "This week" })}
              percentUsed={weekPct}
          ...
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("providerQuota.rollingWindows", { defaultValue: "Rolling windows" })}
              </p>
                    ...
                    <div key={w} className="space-y-1">
                      <div className="flex items-center justify-between gap-2 text-xs">
                        ...
                        <span className="text-muted-foreground font-mono flex-1">
                          {formatTokens(tokens)} {t("providerQuota.tok", { defaultValue: "tok" })}
                        </span>
        ...
        {totalSubRuns > 0 && (
          <>
            ...
            <div className="space-y-2">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("providerQuota.subscription", { defaultValue: "Subscription" })}
              </p>
              <p className="text-xs text-muted-foreground">
                <span className="font-mono text-foreground">{totalSubRuns}</span> {t("providerQuota.runs", { defaultValue: "runs" })}
                {" · "}
                {totalSubTokens > 0 && (
                  <>
                    <span className="font-mono text-foreground">{formatTokens(totalSubTokens)}</span> {t("providerQuota.total", { defaultValue: "total" })}
                    {" · "}
                  </>
                )}
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span> {t("providerQuota.in", { defaultValue: "in" })}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span> {t("providerQuota.out", { defaultValue: "out" })}
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("providerQuota.subscriptionShare", { defaultValue: "{{percent}}% of token usage via subscription", percent: Math.round(subSharePct) })}
                  </p>
                </>
              )}
            </div>
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-3 shrink-0 tabular-nums text-xs">
                        <span className="text-muted-foreground">
                          {formatTokens(rowTokens)} {t("providerQuota.tok", { defaultValue: "tok" })}
                        </span>
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={t("providerQuota.tokenShareTooltip", { defaultValue: "{{percent}}% of provider tokens", percent: Math.round(tokenPct) })}
                      />
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={t("providerQuota.costShareTooltip", { defaultValue: "{{percent}}% of provider cost", percent: Math.round(costPct) })}
                      />
                    </div>
                  </div>
                );
          ...
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  {t("providerQuota.subscriptionQuota", { defaultValue: "Subscription quota" })}
                </p>
                        ...
                        <div key={qw.label} className="space-y-1">
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            {qw.valueLabel != null ? (
                              ...
                              <span className="font-medium tabular-nums">{t("providerQuota.percentUsed", { defaultValue: "{{percent}}% used", percent: qw.usedPercent })}</span>
                            ) : null}
                          </div>
                          ...
                          {qw.detail ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              {t("providerQuota.resets", { defaultValue: "resets {{date}}", date: new Date(qw.resetsAt).toLocaleDateString(undefined, { month: "short", day: "numeric" }) })}
                            </p>
                          ) : null}
                        </div>
                      );
