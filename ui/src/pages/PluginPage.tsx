...
export function PluginPage() {
  ...
  useEffect(() => {
    if (pageSlot) {
      setBreadcrumbs([
        { label: t("page.plugins", { defaultValue: "Plugins" }), href: "/instance/settings/plugins" },
  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("page.selectCompany", { defaultValue: "Select a company to view this page." })}</p>
      </div>
    );
  }

  if (!contributions) {
    return <div className="text-sm text-muted-foreground">{t("page.loading", { defaultValue: "Loading…" })}</div>;
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          {t("routeConflict", { defaultValue: "Multiple plugins declare the route {{route}}. Use the plugin-id route until the conflict is resolved.", route: pluginRoutePath })}
        </div>
      );
    }
  }
