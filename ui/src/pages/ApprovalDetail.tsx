...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("breadcrumb", { defaultValue: "Approvals" }), href: "/approvals" },
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("detail.approval", { defaultValue: "Approval" }) },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("detail.approveFailed", { defaultValue: "Approve failed" })),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("detail.rejectFailed", { defaultValue: "Reject failed" })),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("detail.revisionFailed", { defaultValue: "Revision request failed" })),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("detail.resubmitFailed", { defaultValue: "Resubmit failed" })),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("detail.commentFailed", { defaultValue: "Comment failed" })),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("detail.deleteFailed", { defaultValue: "Delete failed" })),
  });

  if (isLoading) return <PageSkeleton variant="detail" />;
  if (!approval) return <p className="text-sm text-muted-foreground">{t("detail.notFound", { defaultValue: "Approval not found." })}</p>;
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("detail.reviewLinkedIssues", { defaultValue: "Review linked issues" })
              : t("detail.reviewLinkedIssue", { defaultValue: "Review linked issue" }),
          to: `/issues/${primaryLinkedIssue.identifier ?? primaryLinkedIssue.id}`,
        }
      : linkedAgentId
        ? {
            label: t("detail.openHiredAgent", { defaultValue: "Open hired agent" }),
            to: `/agents/${linkedAgentId}`,
          }
        : {
            label: t("detail.backToApprovals", { defaultValue: "Back to approvals" }),
            to: "/approvals",
          };
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("detail.approvalConfirmed", { defaultValue: "Approval confirmed" })}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("detail.agentNotified", { defaultValue: "Requesting agent was notified to review this approval and linked issues." })}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          {approval.requestedByAgentId && (
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground text-xs">{t("detail.requestedBy", { defaultValue: "Requested by" })}</span>
              <Identity
          ...
          >
            ...
            {t("detail.seeFullRequest", { defaultValue: "See full request" })}
          </button>
          {showRawPayload && (
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("detail.decisionNote", { defaultValue: "Decision note:" })} {approval.decisionNote}</p>
          )}
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("detail.linkedIssues", { defaultValue: "Linked Issues" })}</p>
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t("detail.linkedIssuesHint", { defaultValue: "Linked issues remain open until the requesting agent follows up and closes them." })}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          {isActionable && !isBudgetApproval && (
            <>
              ...
              >
                {t("detail.reject", { defaultValue: "Reject" })}
              </Button>
            </>
          )}
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("detail.resolveBudget", { defaultValue: "Resolve this budget stop from the budget controls on" })} <Link to="/costs" className="underline underline-offset-2">/costs</Link>.
            </p>
          )}
          {approval.status === "pending" && (
            ...
            >
              {t("detail.requestRevision", { defaultValue: "Request revision" })}
            </Button>
          )}
          {approval.status === "revision_requested" && (
            ...
            >
              {t("detail.markResubmitted", { defaultValue: "Mark resubmitted" })}
            </Button>
          )}
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("detail.confirmDeleteAgent", { defaultValue: "Delete this disapproved agent? This cannot be undone." }))) return;
                deleteAgentMutation.mutate(linkedAgentId);
              }}
              disabled={deleteAgentMutation.isPending}
            >
              {t("detail.deleteDisapprovedAgent", { defaultValue: "Delete disapproved agent" })}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("detail.comments", { defaultValue: "Comments" })} ({comments?.length ?? 0})</h3>
        ...
        <Textarea
          ...
          placeholder={t("detail.addCommentPlaceholder", { defaultValue: "Add a comment..." })}
          rows={3}
        />
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? t("detail.posting", { defaultValue: "Posting…" }) : t("detail.postComment", { defaultValue: "Post comment" })}
          </Button>
        </div>
      </div>
    </div>
